...
export default function Board({
  ...
  // Positions confirmed correct (green) in any submitted row — pre-filled and
  // shown as "revealed" in the current row.
  const confirmed = new Set<number>();
  for (let r = 0; r < guesses.length; r++) {
    for (let c = 0; c < len; c++) {
      if (guessStates[r]?.[c] === "correct") confirmed.add(c);
    }
  }
    ...
    const tiles = Array.from({ length: len }, (_, c) => {
      ...
      if (targetWord[c] === " ") {
        ...
        if (c === 0) {
          ...
          // A confirmed position still holding its correct letter shows as
          // "revealed"; if the player overwrote it, it reverts to neutral.
          state =
            confirmed.has(c) && currentInput[c] === targetWord[c]
              ? "revealed"
              : "tbd";
        }
  ...
  return (
    <div
      id="board"
      data-size={sizeClass}
      style={{ "--cols": len } as React.CSSProperties}
    >
      {rows.map((row) => (
